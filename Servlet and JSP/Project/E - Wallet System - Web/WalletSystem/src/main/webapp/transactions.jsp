...
if (transactions != null) {
	for (Transaction tx : transactions) {
		...
		if (t.equals("DEPOSIT") || t.equals("TRANSFER_IN"))
	totalDeposited += tx.getAmount();
		if (t.equals("WITHDRAW") || t.equals("TRANSFER_OUT"))
	...
	<div class="sidebar">
		...
		<div class="nav-section" style="padding-top: 8px;">
			<span class="nav-lbl">Admin</span> <a
				href="AdminController?action=showDashboard" class="nav-item"> <i
				class="ti ti-shield-check"></i> Admin Dash Board
			</a>
		</div>
		<%
		}
		...
		<div class="nav-section">
			<span class="nav-lbl">My Wallet</span> <a href="DashboardController"
				class="nav-item"><i class="ti ti-home"></i> Dash Board</a> <a
				...
				href="WalletController?action=showTransactionsPage"
				class="nav-item active"><i class="ti ti-history"></i>
				Transactions</a>
		</div>
		<div class="nav-section">
			<span class="nav-lbl">Account</span> <a
				href="AccountController?action=showProfilePage" class="nav-item"><i
				class="ti ti-user"></i> Profile</a> <a
				href="AccountController?action=showChangePasswordPage"
				class="nav-item"><i class="ti ti-lock"></i> Change Password</a> <a
				href="AccountController?action=showDeletePage"
				class="nav-item danger"><i class="ti ti-trash"></i> Delete
				Account</a>
		</div>
	...
	<div class="main">
		<div class="topbar">
			...
			<div class="topbar-right">
				<a href="#" onclick="confirmLogout()"
					style="display: flex; align-items: center; gap: 5px; padding: 6px 13px; background: #FFFFFF; border: 1px solid rgba(0, 0, 0, 0.09); border-radius: 8px; font-size: 12px; font-weight: 500; color: #3A3A3C; text-decoration: none; box-shadow: 0 1px 2px rgba(0, 0, 0, 0.04);">
					<i class="ti ti-logout" style="font-size: 14px; color: #AEAEB2;"></i>
					Sign out
				</a>
			</div>
		</div>
		...
		<div class="content">
			...
			<div class="strip">
				<div class="sum-card">
					...
					<div>
						<div class="sum-num">
							+<%=String.format("%,.2f", totalDeposited)%></div>
				...
				<div class="sum-card">
					...
					<div>
						<div class="sum-num">
							−<%=String.format("%,.2f", totalWithdrawn)%></div>
				...
				<div class="sum-card">
					...
					<div>
						<div class="sum-num"><%=transferCount%>
							times
						</div>
			...
			<div class="table-wrap"
				style="display: flex; flex-direction: column; max-height: calc(100vh - 220px);">
				<div class="tbl-head">
				...
				<div style="overflow-y: auto; flex: 1;">
					<table class="tx">
						...
						<tbody id="tx-body">
							...
							int idx = 1;
							for (Transaction tx : transactions) {
								String type = tx.getType().toUpperCase().replace(" ", "_");
								...
								String badgeClass = type.equals("DEPOSIT")
								? "b-dep"
								: type.equals("WITHDRAW") ? "b-wit" : (type.equals("TRANSFER_IN") ? "b-in" : "b-out");
								String displayType = tx.getType();
							%>
							...
							<tr data-t="<%=dataType%>">
								...
								<td style="text-align: right;"><span
									class="tx-amt <%=isPos ? "pos" : "neg"%>"> <%=isPos ? "+" : "−"%><%=String.format("%,.2f", tx.getAmount())%>
								</span></td>
